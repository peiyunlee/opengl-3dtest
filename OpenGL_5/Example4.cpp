...
void init( void )
{
	...
	// 產生所需之 Model View 與 Projection Matrix
	point4  eye(g_fRadius*sin(g_fTheta)*sin(g_fPhi), g_fRadius*cos(g_fTheta)+2.0f, g_fRadius*sin(g_fTheta)*cos(g_fPhi), 1.0f);
	//point4  eye(g_fRadius*sin(g_fTheta)*sin(g_fPhi), g_fRadius*cos(g_fTheta)-3.0f, g_fRadius*sin(g_fTheta)*cos(g_fPhi), 1.0f);
...
#endif
	g_pFloor->SetShader();
	g_pFloor->SetTRSMatrix(Scale(50,1,50));
	g_pFloor->SetShadingMode(GOURAUD_SHADING);
	//g_pFloor->SetTiling(25,25);
	// 設定貼圖
